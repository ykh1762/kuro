...
<div class="email-app">
	...
	<form:form action="/mail/write" method="post" id="mailForm"
		modelAtribute="mailVO" enctype="multipart/form-data">
		...
		<div class="right-part mail-list bg-white overflow-auto" style="width: calc(100% - 200px); margin-left: 200px;">
			...
			<div class="card-body">
				...
				<div class="parent" style="display: flex;">
					...
					<input type="text" name="empName" class="form-control" placeholder="받는사람" style="border-color: #6e6e6e;"/>
    	</div>
				...
				<input type="text" class="form-control" placeholder="제목"
					style="border-color: #6e6e6e;" name="mailTitle" /> <br>
				<div class="mb-3">
					<label for="fileInput" class="form-label">파일 선택</label>
    <input type="file" class="form-control" id="fileInput" name="fileInput" style="border-color: #6e6e6e;">
				
				</div>
				
				<div class="col-12" style="border-color: #6e6e6e;">
					<div class="card">
						<div class="border-bottom title-part-padding"></div>
						<div class="card-body">
							<textarea cols="80" style="border-color: #6e6e6e;" id="testedit1" name="mailContent" rows="10"
								title="메일 내용을 입력해 주세요." data-sample="2" data-sample-short>
                  </textarea>
						</div>
					</div>
				</div>
		...
		<div class="modal-content">
				...
				<div class="bootstrap-duallistbox-container row moveonselect">
					<div class="box1 col-md-6">
						<label for="bootstrap-duallistbox-nonselected-list_"
							...
							placeholder="Search" style="border: 2px solid #adabab;"> <br>
						<div class="btn-group buttons" style="width: 225px;">
							<button type="button" class="btn moveall btn-default"
								title="Move all" style="height: 28px; background-color: #FFFFFF;">
							...
							<button type="button" class="btn move btn-default"
								title="Move selected" style="height: 28px; background-color: #FFFFFF;">
								<i class="fas fa-angle-right" style="color: #6C757D;"></i>
							</button>
						</div>
						<select multiple="multiple"
							id="bootstrap-duallistbox-nonselected-list_" class="form-control"
							name="_helper1" style="height: 101.333px; background-color: #D2E0F5; color: #000000;">
					...
					<div class="box2 col-md-6">
						...
						<div class="btn-group buttons" style="width: 225px;">
							<button type="button" class="btn remove btn-default"
								title="Remove selected" style="height: 28px; background-color: #FFFFFF;">
								<i class="fas fa-angle-left" style="color: #6C757D;"></i>
							</button>
							<button type="button" class="btn removeall btn-default"
								title="Remove all" style="height: 28px; background-color: #FFFFFF;">
						...
						<select multiple="multiple"
							...
							name="_helper2" style="height: 101.333px; background-color: #D2E0F5; color: #000000;">
						</select>
					</div>
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" id="btnAddress"
					class="btn waves-effect waves-light btn-primary">확인</button>
				<button type="button" class="btn waves-effect waves-light btn-dark" data-bs-dismiss="modal">
					닫기</button>
			</div>
		</div>
	</div>
...
$(document).ready(function(){
    $("#btnRegister").click(function(){
        window.location.href = "/mail/process";
    });
});

//검색어 입력 상자와 드롭다운 목록 요소 가져오기
var searchInput = document.querySelector('.filter');
var selectBox = document.querySelector('#bootstrap-duallistbox-nonselected-list_');

// 검색어 입력 상자에 입력된 값이 변경될 때마다 이벤트 처리
searchInput.addEventListener('input', function() {
  var searchValue = searchInput.value.toLowerCase(); // 입력된 검색어 소문자로 변환

  // 드롭다운 목록에서 각 옵션을 검사하고 일치하는 값을 보이거나 숨깁니다.
  Array.from(selectBox.options).forEach(function(option) {
    var optionText = option.text.toLowerCase();
    var optionVisible = optionText.includes(searchValue);
    option.style.display = optionVisible ? 'block' : 'none';
  });
});
