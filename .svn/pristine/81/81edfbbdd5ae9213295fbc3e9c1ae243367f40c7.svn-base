	...
	<!-- Right Part -->
		...
		<div class="table-responsive">
			...
			<table class="table table-striped table-bordered display text-nowrap" id="custom_tool_ele">
				...
				<tbody>
					<c:forEach items="${updateMailList }" var="mail" varStatus="i">
						<tr>
						<td style="text-align: center;" data-code="${mail.mailCode }"  class="font-medium link">${empList.get(i.index).empName}</td>
						<td style="text-align: center; text-overflow: ellipsis; overflow:hidden; white-space: nowrap;">${mail.mailTitle }</td>
						<td style="text-align: center;"><fmt:formatDate value="${mail.mailDate }" pattern="yyyy/MM/dd"></fmt:formatDate></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
